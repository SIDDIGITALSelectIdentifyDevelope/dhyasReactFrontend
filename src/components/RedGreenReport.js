...
function RedGreenReport() {
  ...
  useEffect(() => {
    ...
    const counts = filteredVoters.reduce((totals, voter) => {
      const gender = (voter.Gender || '').toLowerCase();
      if (gender === 'male') totals.male += 1;
      else if (gender === 'female') totals.female += 1;
      else if (gender === 'transgender') totals.transgender += 1;
      totals.total += 1;
      return totals;
  ...
  const printTable = () => {
    ...
    printWindow.document.write('<th>Constituency</th><th>Ward No</th><th>Votting Boothe Name</th><th>Epic No</th><th>Name</th><th>Middle Name</th><th>Gender</th><th>Age</th><th>English Name</th><th>Marathi Name</th></tr></thead><tbody>');
