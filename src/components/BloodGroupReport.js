...
function BloodGroupReport() {
  ...
  const [genderTotals, setGenderTotals] = useState({ male: 0, female: 0, transgender: 0, total: 0 });
  ...
  const calculateGenderTotals = (votersList) => {
    ...
    setTotalPages(Math.ceil(votersList.length / votersPerPage)); // Update total pages
  };
  ...
  useEffect(() => {
    ...
    calculateGenderTotals(filteredVoters); // Recalculate gender totals
  }, [filteredVoters]);
