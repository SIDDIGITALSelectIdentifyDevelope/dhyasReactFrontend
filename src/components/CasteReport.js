...
function CasteReport() {
  const [voters, setVoters] = useState([]);
  const [currentVoters, setCurrentVoters] = useState([]); // Define currentVoters here
  ...
  useEffect(() => {
    ...
    const totals = filteredVoters.reduce((totals, voter) => {
    ...
    setCurrentVoters(filteredVoters.slice(
      (currentPage - 1) * votersPerPage,
      currentPage * votersPerPage
    ));
  }, [filters, currentPage, voters]);
